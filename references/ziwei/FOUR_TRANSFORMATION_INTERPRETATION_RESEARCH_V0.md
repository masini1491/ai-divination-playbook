...
## Production-admission candidate binding

The production candidate `sihua_v1` binds only the three claims in
`ziwei_interpretation_claim_registry_sihua_v0.json`.

Admission requires:

```text
explicit optional module = sihua_v1
sihua_profile = sihua.default_v1
fact_available:sihua
fact_available:star_locations
exact sihua:<stem>:<transform>:<star>
exact star_branch:<star>:<branch>
```

The deterministic provider itself grants no interpretation authority. Claim
authority comes from `ZIWEI_SIHUA_ADMISSION_V1.json` + production runtime
allowlisting of the historical research registry while preserving
`production_routable=false`.

No other transformed-star claim, generic 祿／權／科／忌 outcome dictionary, or
cross-profile averaging is admitted by this gate.
