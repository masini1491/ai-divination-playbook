"""REFERENCE-ONLY AST-P1-030 Osculating / True Black Moon Lilith research.

Candidate identity:
    black_moon_lilith_osculating_lrl_aengine_v0

Definition:
    instantaneous lunar osculating apogee from the Moon geocentric
    position/velocity state using the Laplace-Runge-Lenz eccentricity vector.

Research input authority:
    astronomy-engine==2.1.19 GeoMoonState() in J2000 mean equatorial (EQJ),
    position in AU and velocity in AU/day.

Output frame:
    true ecliptic/equinox of date (ECT), normalized mod 360 degrees.

This module is REFERENCE-ONLY. It does not expose a production Astrology provider,
does not grant Moon state-vector production admission, and exposes no bare
"Lilith" alias.
"""
...
class ResearchInputError(RuntimeError):
    """Required research state input is unavailable or invalid."""
...
def _lrl_eccentricity(
    ...
    *,
    ...
    if not math.isfinite(mu) or mu <= 0.0:
        raise ResearchInputError("Earth-Moon gravitational parameter must be positive")
    r_mag = math.sqrt(x*x + y*y + z*z)
    if not math.isfinite(r_mag) or r_mag <= 0.0:
        raise ResearchInputError("invalid lunar position magnitude")
    ...
    if not math.isfinite(e_mag) or e_mag <= 1.0e-12:
        raise ResearchInputError("degenerate lunar eccentricity vector")
    return ex, ey, ez
...
def _apogee_longitude_from_eqj_evector(
    ...
    perigee_ect = astronomy.RotateVector(
...
def osculating_lilith_from_state(
    ...
    *,
    ...
    try:
        ...
        raise ResearchInputError("required GeoMoonState research input unavailable") from exc
    e = _lrl_eccentricity(
...
def osculating_lilith_from_position_finite_difference(
    ...
    *,
    ...
    if not math.isfinite(dt_days) or dt_days <= 0.0:
        raise ResearchInputError("dt_days must be positive")
    try:
        ...
        raise ResearchInputError("required GeoMoon research input unavailable") from exc
