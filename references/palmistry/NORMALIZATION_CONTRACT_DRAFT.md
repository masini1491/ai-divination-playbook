...
## 10. Draft deterministic validation matrix

未來若建立 research-only validator，最低應測：

| Case | Expected invariant / behavior |
|---|---|
| translation | canonical x/y unchanged |
| in-plane rotation | canonical x/y unchanged |
| uniform scale | canonical x/y unchanged |
| image mirror with landmark labels preserved | canonical anatomical x direction remains index-side positive |
| detector-only mirror + inverse | round-trip geometry matches pre-mirror geometry |
| crop + resize + inverse | endpoints / paths round-trip within tolerance |
| missing L0/L5/L17 | normalization fail closed |
| palm height / width near zero | normalization fail closed |
| material foreshortening without rectifier | fine source-region mapping unresolved |
| multiple hands | only selected target landmarks enter basis |
| detector heart/head/life labels | remain tool-local, no Chinese mapping |

## 11. Adoption decision

本輪只建立 research contract：
...
- deterministic repository tool；
- numeric tolerance；
- landmark-version compatibility matrix；
- cross-device repeatability；
- detector accuracy acceptance threshold。

下一個合理 research node 是先用 synthetic landmark fixtures 建立小型 deterministic normalization probe，驗證上述 translation / rotation / scale / mirror / fail-closed 性質，再決定是否值得形成正式 implementation owner。
