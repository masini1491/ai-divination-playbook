...
- stochastic 三錢起卦 → `runtime/casting/core.py`（canonical stochastic core）→ `runtime/casting/randomizer.py`（full API / CLI adapter）
...
> **Canonical stochastic core 決定六爻原始 6/7/8/9；full Runtime adapter 只轉接／輸出同一 execution；deterministic engine 決定可驗證盤面 facts；Playbook 決定題目責任、用神 responsibility 與解讀治理。**
...
本 Playbook 預設的 AI Runtime 六爻起卦方式是 logical runtime identity `divination-casting-randomizer-python` 的 canonical three-coin cast；canonical stochastic core 位於 `runtime/casting/core.py`，正式 entrypoint 是 `core.execute_stochastic()`；`runtime/casting/randomizer.py` 是 full API / CLI adapter：
