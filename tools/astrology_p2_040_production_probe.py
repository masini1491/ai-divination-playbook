...
OUTPUT = Path(os.environ.get("AST_P2_040_OUTPUT", "tmp/astrology-p2-040-artifact"))
    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "ai-divination-playbook-ast-p2-040-production-probe/1"},
            )
...
def write_dataset(coeffs, signature, source_digests):
    ...
    for shard_index, first_segment in enumerate(range(0, SEGMENT_COUNT, SEGMENTS_PER_SHARD)):
        ...
        for seg in range(first_segment, first_segment + count):
            for object_id, _ in OBJECTS:
                raw.extend(struct.pack("<8d", *coeffs[object_id][seg]))
        data = bytes(raw)
    ...
    manifest = {
        ...
        "representation": {
            ...
            "float_encoding": "IEEE-754 float64 little-endian",
...
def eval_from_dataset(root: Path, manifest: dict, object_id: str, when: dt.datetime):
    ...
    coeff = list(struct.unpack("<8d", rec))
...
def main():
    ...
    result = {
        "schema_name": "astrology_p2_040_production_probe",
        "schema_version": "1.0.0",
        "authority": "PRODUCTION_ADMISSION_EVIDENCE_CANDIDATE",
        "production_admission": "NOT_GRANTED_BY_PROBE",
        "contract_commit": "bdc7a104671d82fe8f0ba26b6e2d4558dc43725f",
    ...
    print("ASTROLOGY_P2_040_JSON_BEGIN")
    print(json.dumps(result, indent=2, sort_keys=True))
    print("ASTROLOGY_P2_040_JSON_END")
