...
DYNAMIC_V3_RUNTIME_ID="ziwei-production-runtime-v3"
...
@dataclass(frozen=True)
class ZiWeiDynamicV3Request:
    request_id: str
    birth: GregorianBirthInput | NormalizedNatalInput
    gender: Literal["male","female"]
    target_lunar_year: int
    temporal_scope: Literal["decadal","yearly"]
    requested_subjects: tuple[str,...] = ()
    enabled_source_ids: tuple[str,...] = ()
    ...
    def validate(self)->None:
        if not isinstance(self.request_id,str) or not self.request_id.strip():
            raise ValueError("request_id is required")
        if self.temporal_scope not in ("decadal","yearly"):
            raise ValueError(f"unsupported temporal_scope: {self.temporal_scope}")
        if not isinstance(self.birth,(GregorianBirthInput,NormalizedNatalInput)):
            raise ValueError("birth must be GregorianBirthInput or NormalizedNatalInput")
        if self.gender not in ("male","female"):
...
def run_ziwei_dynamic_v3(request:ZiWeiDynamicV3Request)->dict[str,Any]:
    request.validate()
    natal,calendar=_normalize_birth(request.birth)
    if request.temporal_scope=="decadal":
        calculation=calculate_decadal(natal,DecadalTarget(gender=request.gender,target_lunar_year=request.target_lunar_year))
        scope="bounded_decadal_calculation_v1"
        reason="ZW-P1-040 decadal interpretation not yet admitted"
    elif request.temporal_scope=="yearly":
        calculation=calculate_yearly(natal,YearlyTarget(gender=request.gender,target_lunar_year=request.target_lunar_year))
    ...
    result={
        "schema_name":"ziwei_dynamic_result","schema_version":"3.0.0",
        "runtime":{
            "runtime_id":DYNAMIC_V3_RUNTIME_ID,"runtime_version":DYNAMIC_V3_RUNTIME_VERSION,
            "request_schema":DYNAMIC_V3_REQUEST_SCHEMA,"result_schema":DYNAMIC_V3_RESULT_SCHEMA,
...
def run_ziwei_dynamic_v3_transport(payload:dict[str,Any])->dict[str,Any]:
    if not isinstance(payload,dict):
        raise ValueError("dynamic v3 request transport must be an object")
    allowed={"schema_name","schema_version","request_id","temporal_scope","birth","gender","target_lunar_year","requested_subjects","enabled_source_ids"}
    unknown=set(payload)-allowed; missing=allowed-set(payload)
    if unknown: raise ValueError(f"unknown dynamic v3 request fields: {','.join(sorted(unknown))}")
    if missing: raise ValueError(f"missing dynamic v3 request fields: {','.join(sorted(missing))}")
    if payload["schema_name"]!="ziwei_dynamic_request" or payload["schema_version"]!="3.0.0":
        raise ValueError("unsupported Zi Wei dynamic v3 request schema")
    if payload["temporal_scope"] not in ("decadal","yearly"):
        raise ValueError(f"unsupported temporal_scope: {payload['temporal_scope']}")
    birth=payload["birth"]
