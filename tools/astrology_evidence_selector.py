...
def _validate_request(data: Any) -> dict[str, Any]:
    ...
    for index, raw in enumerate(data["fact_selectors"]):
        ...
        if kind == "object":
            ...
            allowed = common | {"event_kind", "moving_body", "natal_target", "aspect", "passage_index", "transition", "ingress_type", "from_sign", "to_sign", "from_house", "to_house", "house_number", "house_system", "exact_time_utc"}
        else:
...
def _selector_applicability(selector: dict[str, Any]) -> set[str]:
    ...
    if kind == "house":
        ...
        tags.add({"transit_to_natal": "transit-to-natal", "station": "station", "ingress": "ingress", "house_ingress": "house ingress", "house_context": "house context"}[event_kind])
        ...
        if event_kind == "house_context" and "house_number" in selector:
            tags.add(f"house {selector['house_number']}")
