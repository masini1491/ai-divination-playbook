#!/usr/bin/env python3
"""Controlled build-time generator for Astrology extended ephemeris v1.

This generator is the deterministic source-to-corpus owner for the admitted
C1 Chebyshev / float32 transport candidate. It may contact NASA/JPL Horizons
only during controlled build/publish execution. Ordinary Astrology runtime
must never invoke network generation.
"""
    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "ai-divination-playbook-extended-ephemeris-generator/1"},
            )
...
def write_dataset(coeffs, signature, source_digests):
    ...
    manifest = {
        ...
        "production_admission": "GENERATED_CANDIDATE_ONLY",
        "source": {
        ...
        "representation": {
            ...
            "record_layout": "segment-major, object-major, 8 float32 coefficients",
        },
        "dataset": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate the frozen Astrology extended ephemeris corpus.")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--write", action="store_true")
    parser.add_argument("--expected-digest", required=True)
    args = parser.parse_args()
    if not args.write:
        raise SystemExit("--write is required for controlled corpus generation")
    ...
    actual = manifest["dataset"]["sha256"]
    if actual != args.expected_digest:
        raise RuntimeError(f"generated dataset digest mismatch: {actual} != {args.expected_digest}")

    summary = {
        "generator_id": "astrology-extended-ephemeris-generator-v1",
        "dataset_id": manifest["dataset_id"],
        "representation_id": manifest["representation"]["id"],
        "dataset_sha256": actual,
        "raw_byte_size": manifest["dataset"]["raw_byte_size"],
        "shard_count": manifest["dataset"]["shard_count"],
        "observed_signature": manifest["source"]["observed_signature"],
        "object_source_grid_sha256": manifest["source"]["object_source_grid_sha256"],
        "root": str(root),
    }
