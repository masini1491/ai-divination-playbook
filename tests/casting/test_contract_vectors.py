...
VECTORS = json.loads((CASTING_ROOT / "contract_vectors.json").read_text(encoding="utf-8"))
...
class CrossRuntimeContractTests(unittest.TestCase):
    def test_tarot_deck_matches_shared_vector_in_python_and_web(self):
        ...
        web_deck = _extract_json_array("majors") + [suit + rank for suit in _extract_json_array("suits") for rank in _extract_json_array("ranks")]
