...
class RandomizerTests(unittest.TestCase):
    ...
    def test_all_hexagram_pairs_are_covered(self):
        pairs = {upper + lower for upper in set(randomizer.TRIGRAM.values()) for lower in set(randomizer.TRIGRAM.values())}
    ...
    def test_liuyao_coin_resolution_covers_four_line_values(self):
        ...
        for coins, expected in cases.items():
            ...
            self.assertEqual((result["value"], result["yin_yang"], result["changing"], result["line_type"]), expected)
    ...
    def test_liuyao_cast_has_six_bottom_to_top_lines(self):
        ...
        self.assertEqual([line["position_name"] for line in result["lines"]], list(randomizer.LIUYAO_POSITION_NAMES))
    ...
    def test_ai_json_cli_is_compact_and_parseable(self):
        completed = subprocess.run([sys.executable, str(CASTING_ROOT / "randomizer.py"), "tarot", "--count", "5", "--repeat", "3", "--format", "ai-json"], check=True, capture_output=True, text=True)
    ...
    def test_liuyao_cli_json_is_parseable(self):
        completed = subprocess.run([sys.executable, str(CASTING_ROOT / "randomizer.py"), "liuyao", "--format", "json"], check=True, capture_output=True, text=True)
        payload = json.loads(completed.stdout)
